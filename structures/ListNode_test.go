...
func Test_l2s(t *testing.T) {
	...
	ast.Equal([]int{}, List2Ints(nil), "Enter nil, no return []int{}")
	...
	ast.Equal([]int{1, 2, 3}, onw2three, "Was not successfully converted to []int")
...
func Test_s2l(t *testing.T) {
	ast := assert.New(t)
	ast.Nil(Ints2List([]int{}), "Enter []int{}, no nil is returned")

	ln := Ints2List([]int{1, 2, 3, 4, 5, 6, 7, 8, 9})
	i := 1
	for ln != nil {
		ast.Equal(i, ln.Val, "The corresponding value is incorrect")
		ln = ln.Next
		i++
	}
}
